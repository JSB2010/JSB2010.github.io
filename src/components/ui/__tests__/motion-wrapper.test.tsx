...
describe('MotionWrapper Component', () => {
  ...
  it('forwards animation props to motion component', () => {
    ...
    expect(motionElement).toHaveAttribute('initial', JSON.stringify({ opacity: 0 }));
    expect(motionElement).toHaveAttribute('animate', JSON.stringify({ opacity: 1 }));
    expect(motionElement).toHaveAttribute('transition', JSON.stringify({ duration: 0.3 }));
  });
